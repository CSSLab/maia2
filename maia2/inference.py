...
def _masked_softmax(logits, legal_moves):
    ...
    if rows_without_legal_moves.any():
        row_indices = (
            rows_without_legal_moves.nonzero(as_tuple=False).flatten().tolist()
        )
        raise ValueError(
            f"Cannot run inference without legal moves (batch rows: {row_indices})."
        )
...
def preprocessing(fen, elo_self, elo_oppo, elo_dict, all_moves_dict):
    ...
    if not legal_moves_idx:
        raise ValueError(
            f"Cannot run inference on a position without legal moves: {fen}"
        )
...
class TestDataset(torch.utils.data.Dataset):
    def __init__(self, data, all_moves_dict, elo_dict):
        required_columns = ["board", "move", "active_elo", "opponent_elo"]
        missing_columns = [
            column for column in required_columns if column not in data.columns
        ]
        if missing_columns:
            raise ValueError(
                "Inference data is missing required columns: "
                + ", ".join(missing_columns)
            )

        self.all_moves_dict = all_moves_dict
        self.data = data.loc[:, required_columns].values.tolist()
    ...
    with torch.no_grad():
        for fens, boards, elos_self, elos_oppo, legal_moves in dataloader:
            ...
            for i in range(len(fens)):
                ...
                ranked_moves = sorted(
                    (
                        (legal_moves_mirrored[j], probs[i][move_index])
                        for j, move_index in enumerate(legal_move_indices)
                    ),
                    key=lambda item: item[1],
                    reverse=True,
                )
                move_probs_each = {
                    move: round(probability, 4) for move, probability in ranked_moves
                }
...
def inference_batch(data, model, verbose, batch_size, num_workers):
    ...
    if data.empty:
        raise ValueError("Inference data must contain at least one position.")
    ...
    for i in range(len(data)):
        # Dictionaries retain the ordering established from unrounded model
        # probabilities in ``get_preds``. This avoids changing the predicted
        # move when two displayed probabilities tie after four-place rounding.
        highest_prob_move = next(iter(move_probs[i]))
...
def inference_each(model, prepared, fen, elo_self, elo_oppo):
    ...
    legal_move_indices = legal_moves[0].nonzero(as_tuple=True)[0].cpu().tolist()
    legal_moves_mirrored = []
    for move_idx in legal_move_indices:
        move = all_moves_dict_reversed[move_idx]
        if black_flag:
    ...
    ranked_moves = sorted(
        (
            (legal_moves_mirrored[j], probs[0][move_index])
            for j, move_index in enumerate(legal_move_indices)
        ),
        key=lambda item: item[1],
        reverse=True,
    )
    move_probs = {move: round(probability, 4) for move, probability in ranked_moves}
